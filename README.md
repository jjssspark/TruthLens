...
- **영상 판별(FR-01)은 학습된 딥페이크 모델이 아니라 로컬 휴리스틱입니다.** `ai_models/video_detector.py`가 OpenCV로 프레임을 최대 16장 균등 샘플링해 이미지 판별과 같은 픽셀 휴리스틱(노이즈 균일도·엣지·색상 분포)을 프레임마다 적용하고, 프레임 간 변화량으로 시간적 일관성을 추가로 봅니다. 외부 API 키는 필요 없습니다. 다만 상용 딥페이크 탐지 모델과 같은 정확도를 기대할 수는 없으며, 학습 모델 연동은 로드맵입니다.

### 알려진 한계

- **영상 판별(FR-01)이 학습 모델이 아닌 휴리스틱** — 위 항목 참고.
