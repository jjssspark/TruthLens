...
**목표**
- 영상·이미지·텍스트(뉴스·논문)에 대한 AI 생성 여부 판별 정확도 85% 이상 달성 (이미지는 자체 표본에서 94.3%, **영상은 공개 데이터셋 27개 실측 37%로 미달** — 아래 '알려진 한계' 참고)
...
| 기능 | 도메인 | 설명 |
| --- | --- | --- |
| FR-01 영상 AI 생성 판별 | 영상 | URL(YouTube, Vimeo, 직접 링크) 또는 파일(MP4/AVI/MOV/WEBM, PRD 설계는 500MB·10분 — **배포본은 앞단 프록시 제한으로 96MB까지**) 입력 → AI 생성 신뢰 점수, 딥페이크 탐지, 프레임 단위 의심 구간 하이라이트, 판별 근거 요약 제공 |
| FR-02 이미지 AI 생성 판별 | 이미지 | 이미지(JPG/PNG/WebP/GIF, 최대 20MB, PRD 설계는 다중 업로드 10장 — **현재 1장씩으로 임시 제한**) → AI 생성 신뢰 점수, 의심 영역 히트맵, 픽셀 패턴 이상 탐지, EXIF 메타데이터 분석 |
...
- 영상이 사람 촬영본을 AI 62%로 판정하던 오탐을, 얼굴 조작 탐지 모델을 일반 프레임에 쓰고 있었다는 데까지 추적해 이미지 앙상블로 교체했습니다.
- 배포본을 로그만 보고 진단하다 원인을 두 번 잘못 짚었고, 그 뒤 `/diagnostics`를 만들어 배포된 코드와 모델 상태를 밖에서 확인할 수 있게 했습니다. 틀린 결론은 지우지 않고 정정 이력을 남겼습니다.
- 전체 28건(재현 조건·시도했지만 안 된 것 포함)은 [`docs/TROUBLESHOOTING.md`](docs/TROUBLESHOOTING.md)에 정리했습니다.
...
- 이미지 판별은 모델 5개를 같은 표본으로 비교해, 정확도는 같은데 실패 방향이 반대인 3개를 골라 중앙값(다수결)으로 묶었습니다.
- 영상 판별은 구현 후 정확도를 실측해 기준선보다 낮은 것을 확인하고, 지우는 대신 실측값을 화면에 공개하는 쪽을 택했습니다.
- 전체 결정 10건(고려한 대안, 근거, 실제 결과, 나중에 뒤집힌 것)은 [`docs/ADR.md`](docs/ADR.md)에 정리했습니다.
