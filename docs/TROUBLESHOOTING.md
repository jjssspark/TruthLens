...
- 검증: 4K 52초 영상(3420x1962) 로컬 실측.
  ...
  원인은 동시 요청 수였다. 콜 하나는 0.5~1.6초인데 18개를 한꺼번에 쏘면 HF가 20초 read timeout을 낸다(18콜 중 2~5개 실패). 동시 실행을 6개로 제한해 실패는 없앴지만, HF 응답 시간 자체가 4~19초로 흔들린다. 동시 실행 수를 3 / 4 / 6 / 9 / 18로 바꿔가며 각각 여러 번 재봤지만 일관된 우열이 나오지 않았다. 같은 설정에서 4.25초와 15.03초가 나온다. 서버 쪽 편차라 코드로 잡을 수 없다.

- 추후 관리:
  - 5회 중 1회에서 점수가 4.3이 아니라 9.2로 나왔다. 모델 하나가 실패해 중앙값이 밀린 것이다. 판정(미탐지)은 그대로지만 숫자는 움직인다. 같은 파일은 해시 캐시로 재분석되지 않아 실사용에서 잘 드러나지 않을 뿐이다.
