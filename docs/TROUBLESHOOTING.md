...
- 추후 관리:
  - **표본이 3장이고 전부 같은 생성기(Google Stitch)의 일러스트다. 이걸로 일반 정확도를 주장할 수 없다.** 실제 사진과 다양한 생성기 결과물로 정량 측정이 필요하다.
  ...
  - 판별 방식이 바뀌었으므로 **기존 Redis 캐시에는 옛 휴리스틱 결과가 남아 있다.** 같은 파일을 다시 올리면 캐시 히트로 옛 점수가 나온다. 배포 후 캐시를 비우거나 캐시 키에 모델 버전을 포함시켜야 한다.
